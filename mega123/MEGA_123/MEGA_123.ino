...
void setup() { 
  ...
  //delay(5000);
  STRIP.begin(); STRIP.setBrightness(255); for (int i=0; i<COUNT_PIXEL; i++) SET_RGB(i, 0, 0, 0); STRIP.show();
/*
...
*/
  ...
  pinMode(PIN_RELE_TELEK, OUTPUT); digitalWrite(PIN_RELE_TELEK, HIGH);  
  ...
  KMPS.attach(PIN_KOMPAS_1); KMPS.write(0); delay(2000); KMPS.detach();
...
void loop() {    
  ...
  if (millis()-TM_Status_Message>500) {
    ...
    switch (Message_Num) {
      ...
      */
      ...
      case 4: { PRINT("{s.fd.s:" + String(!digitalRead(PIN_GERKON_1_DOOR)) + "}");    Message_Num++; break; }
      case 5: { PRINT("{s.h.m:" + String(digitalRead(PIN_DATCHIK_DVIZH)) + "}");    Message_Num++; break; }
      case 6: { PRINT("{s.gd.s:" + String(!digitalRead(PIN_GERKON_GRID_OPEN))+String(!digitalRead(PIN_GERKON_GRID_CLOSE)) + "}");    Message_Num++; break; }
      case 7: { if (!TR_Din_Machine_On) PRINT("{s.dm.s:i}"); else PRINT("{s.dm.s:s}");    Message_Num++; break; }
      case 8: { PRINT("{s.btn.s:" + String(digitalRead(PIN_STOP_GAME_BUTTON)) + "}");    Message_Num++; break; }

      case 9: { PRINT("{s.t.s:" + String(!digitalRead(PIN_TILE_1))+String(!digitalRead(PIN_TILE_2))+String(!digitalRead(PIN_TILE_3))+String(!digitalRead(PIN_TILE_4))+String(!digitalRead(PIN_TILE_5))+String(!digitalRead(PIN_TILE_6)) + "}");    Message_Num++; break; }
      case 10: { PRINT("{s.td.s:" + String(!digitalRead(PIN_GERKON_DOOR_TOMB_OPEN))+String(!digitalRead(PIN_GERKON_DOOR_TOMB_CLOSE)) + "}");    Message_Num++; break; }
      
      case 11: { PRINT("{s.bt.s:" + String(!digitalRead(PIN_GERKON_BURNT_FAKEL)) + "}");    Message_Num++; break; }
      case 12: { PRINT("{s.ar.s:" + String(!digitalRead(PIN_GERKON_ARCHEOLOGST)) + "}");    Message_Num++; break; }
      case 13: { PRINT("{s.cjd.s:" + String(!digitalRead(PIN_GERKON_KANOP_BOX)) + "}");     Message_Num++; break; }
      case 14: { PRINT("{s.an.se:" + String(analogRead(PIN_ANUBIS_NOSE_FOTO)) + "}");     Message_Num++; break; }
      case 15: { PRINT("{s.s.l:" + String(!digitalRead(PIN_GERCON_SARKOFAG_OPEN))+String(!digitalRead(PIN_GERCON_SARKOFAG_CLOSE)) + "}");    Message_Num++; break; }
      case 16: { PRINT("{s.s.ma:" + String(!digitalRead(PIN_GERCON_MASKA)) + "}");    Message_Num++; break; }
      case 17: { PRINT("{s.s.me:" + String(!digitalRead(PIN_GERCON_MEDALLION)) + "}");    Message_Num++; break; }
      case 18: { PRINT("{s.s.sp:" + String(!digitalRead(PIN_GERCON_SFINKS)) + "}");    Message_Num++; break; }
      case 19: { PRINT("{s.dd.s:" + String(!digitalRead(PIN_GERKON_DODEKAEDR_BOX)) + "}");    Message_Num++; break; }
      case 20: { PRINT("{s.sc.s:"+String(!digitalRead(PIN_GERKON_ZHYK))+"}");    Message_Num++; break; }
      case 21: { PRINT("{s.al.s:" + String(!digitalRead(PIN_GERKON_ARTEFAKT_1))+String(!digitalRead(PIN_GERKON_ARTEFAKT_2))+String(!digitalRead(PIN_GERKON_ARTEFAKT_3))+"}");    Message_Num++; break; }
  ...
  if (RS485.available() > 0) {
    while (1) {
      ...
      //ОСВЕЩЕНИЕ
        ...
        else if (String(commandValue.c_str())==String("0")) for (int i=0; i<COUNT_PIXEL; i++) SET_RGB(i, 0, 0, 0);
        STRIP.show();
      }
      //ДЫМОВУХИ
      ...
      */
      else if (command == "ot_cmps") {  KMPS.attach(PIN_KOMPAS_1);  KMPS.write(int(float(commandValue.toInt())/1.5));  TR_Compas=true;  } 
  ...
  */
  ...
  //Мерцание факелов
  static unsigned long TM_Blink_Fakel_12; static unsigned long TM_Blink_Fakel_Tomb_1; static unsigned long TM_Blink_Fakel_Tomb_2; static unsigned long TM_Blink_Fakel_Tomb_3;
