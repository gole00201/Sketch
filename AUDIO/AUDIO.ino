...
//                               ОБЪЕКТЫ                                   |
//-------------------------------------------------------------------------+
RF24 radio(9, 10); //Объект радиомодуля Для MEGA2560 замените на RF24 radio(9,53);
...
void loop() {  
  ...
  */
  ...
  static unsigned long NUM; NUM+=1; static String Last_Command; static unsigned long TM_Status_Message; static String TR_Bomba_Stat; static String TR_Bomba_Pass; static String TR_Bomba_Timer;
  if (millis()-TM_Status_Message>500) {
    ...
    Serial.print(Last_Command); Serial.print("/");
    Serial.print("AUDIO"); Serial.print("/");
    
    Serial.print(radio.begin()); Serial.print("/");
    strcpy(MESSAGE.Whom, "To_Bomba"); strcpy(MESSAGE.Command, "Get_Info"); 
    radio.setChannel(100); // выбираем канал (от 0 до 127)
    radio.setDataRate(RF24_1MBPS); // скорость: RF24_250KBPS, RF24_1MBPS или RF24_2MBPS
    radio.setPALevel(RF24_PA_HIGH); // мощность: RF24_PA_MIN=-18dBm, RF24_PA_LOW=-12dBm, RF24_PA_MED=-6dBM
    radio.enableAckPayload(); // Указываем что в пакетах подтверждения приёма есть блок с пользовательскими данными.
    ...
    delay(100);
    radio.write(&MESSAGE, sizeof(MESSAGE)); 
    if (radio.isAckPayloadAvailable()) { Serial.print(1); radio.read(&BOMBA, sizeof(BOMBA)); } else Serial.print(0);
    Serial.print("/");
    Serial.print(BOMBA.Power); Serial.print("/");
    Serial.print(BOMBA.Status); Serial.print("/");

    for (int i=0; i<sizeof(ARR_MP3_NAME)/sizeof(ARR_MP3_NAME[0]); i++) {
      if (ARR_MP3_NAME[i]!=0 and !ARR[i]) ARR_MP3_NAME[i]=0;
      Serial.print(ARR_MP3_NAME[i]);
